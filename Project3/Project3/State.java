...
*<p>[X]a. Fields: State Name, Capital City, State Abbreviation, State Population, Region, US House Seats
*<p>[{@link #State(String, String, String, int, String, int)}]b. Constructor
*<p>[X]c. Get and set methods for each field
*<p>[{@link #compareTo(State)}]d. Compare method to compare based on State Name (expects a State object as a parameter)
*<p>[{@link #printState()}]e. Method to print a state
* 
...
public class State {
	...
	public String printState(){
		return String.format("%-16s%-16s%-12s%,d\t\t%-10s\t\t%s", name, capital, abb, population, region, seats);
	}
	...
	/**
	 * Compares the state name to another state name string to determine if they are the same
	 * @param stateName the State name as a string that we are trying to compare
	 * @return the value returned is zero if the objects are the same
	 */
	public int compareTo(String stateName){
		return name.compareToIgnoreCase(stateName);
	}
